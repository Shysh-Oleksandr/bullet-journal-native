...
export const fetchAllNotes = createAsyncThunk(
  "journal/fetchAllNotesStatus",
  async ({ user, filter, sort }: IFilterNotes) => {
    ...
    if (response.status === 200 || response.status === 304) {
      ...
      const endNotes: INote[] = notes
        .filter(
          (note) =>
            dateDiffInDays(new Date(note.startDate), new Date(note.endDate)) +
              1 >=
            2
        )
        .map((note) => {
      ...
      notes = [...notes, ...endNotes].filter(
        (note) => note.startDate <= new Date().getTime()
      );
...
export const journalSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchAllNotes.fulfilled, (state, action) => {
      ...
      state.loading = false;
      if (!state.oldestNoteDate)
        state.oldestNoteDate =
          action.payload.oldestNoteDate || state.oldestNoteDate;
    });
